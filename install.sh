...
echo -e "${BLUE}║  스크립트 → 스토리보드 → Remotion          ║${NC}"
echo -e "${BLUE}║  → 렌더링 → 배포                           ║${NC}"
...
SKILLS=(
    "skills/video-orchestra.md|비디오 프로덕션 오케스트레이터"
...
COMMANDS=(
    "commands/video.md|/video — 오케스트레이터"
...
## 비디오 프로덕션
- `/video init [name]`: 새 비디오 프로젝트 초기화
...
- ⚠️ CSS animation/transition/keyframes 절대 금지 — useCurrentFrame() + interpolate()/spring() 사용
...
echo ""
echo "  사용 가능한 명령어:"
echo "    /video init [name]         새 비디오 프로젝트 초기화"
echo "    /video scan                기존 프로젝트 분석"
echo "    /video status              전체 진행 상태 확인"
...
echo ""
echo -e "시작하기:"
echo -e "  ${YELLOW}claude \"/video init my-video\"${NC}"
echo ""
