...
## 실행 절차
1. storyboard/ 에서 스토리보드 읽기
2. 씬별 React 컴포넌트 생성 (src/scenes/Scene{N}-{Name}.tsx)
3. CSS animation 사용 금지 — interpolate/spring만 사용
4. 공통 컴포넌트 재활용 (src/components/)
5. src/Root.tsx에 Composition 등록
6. 타입 안전성 확보 (TypeScript)

## 생성 파일
- `src/scenes/Scene{NN}-{Name}.tsx` — 씬 컴포넌트
- `src/components/{Name}.tsx` — 필요 시 공통 컴포넌트
- `src/Root.tsx` 업데이트 — Composition 등록
