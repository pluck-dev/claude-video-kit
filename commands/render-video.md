...
- 엔트리 파일: `src/index.ts` (루트 모노레포 공유)
- Composition ID: `{name}-full`, `{name}-scene{NN}` 등
...
### 특정 씬 렌더링
```bash
npx remotion render src/index.ts {name}-scene{NN} {OUTPUT}/{name}-scene{NN}.mp4
```
...
## 사전 체크
1. 루트 Remotion 프로젝트 존재 확인 (package.json + remotion 의존성)
2. {VIDEO}/Compositions.tsx에 Composition 등록 확인
3. src/Root.tsx에 이 영상의 Compositions import 확인
4. 에셋 파일 존재 확인 (public/videos/{category}/{name}/)
5. 이전 렌더링 결과 백업 ({OUTPUT}/ 기존 파일)

## 출력
- `{OUTPUT}/{name}.mp4` — 렌더링 결과
- {VIDEO}/VIDEO.md Phase 5a 상태 업데이트

## 트러블슈팅
- `Error: No composition found` → {VIDEO}/Compositions.tsx + src/Root.tsx 확인
- `Error: Could not find file` → public/videos/{category}/{name}/ 에셋 경로 확인
